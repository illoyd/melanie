...
RSpec.describe AddMentionsForContentJob, :with_database do
  ...
  context 'with new hashtags' do
    ...
    it 'creates three hashtags' do
      expect{ subject.perform(content_object._rid) }.to change{ Hashtag.all.count }.from(0).to(3)
    end

    it 'creates #hashtags' do
      expect{ subject.perform(content_object._rid) }.to change{ Hashtag.find_by(text: 'hashtags') }.from(nil)
    end

    it 'creates #hashtagsforlife' do
      expect{ subject.perform(content_object._rid) }.to change{ Hashtag.find_by(text: 'hashtagsforlife') }.from(nil)
    end

    it 'creates #reallycool' do
      expect{ subject.perform(content_object._rid) }.to change{ Hashtag.find_by(text: 'reallycool') }.from(nil)
    end

    it 'creates new mentions' do
      expect( content_object['out_Mentions'] ).to be_nil
      subject.perform(content_object._rid)
  ...
  context 'with new and existing hashtags' do
    ...
    it 'creates two new hashtags' do
      expect{ subject.perform(content_object._rid) }.to change{ Hashtag.all.count }.by(2)
    end

    it 're-uses #hashtags' do
      expect{ subject.perform(content_object._rid) }.not_to change{ Hashtag.find_by(text: 'hashtags')._rid }
    end
